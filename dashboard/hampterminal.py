...
import hmtr_actions as actions 
import asyncio
import time
import queue
...
'''
class hampterminal(App):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.data_queue = queue.Queue()
    CSS_PATH = "tui.tcss"
    TITLE = f"HampTerminal | Cool Guys Look Into the Sun. "
    current_sort_key = 'link_pyth_drift_percent'    
    BINDINGS = [
        ("ctrl+t", "app.toggle_dark", "Dark mode"),
        ("f1", "app.toggle_class('RichLog', '-hidden')", "Logs"),
        ("f2", "sort_table('link_pyth_drift_percent')", "Sort Price Diff"),
    ...
    sort_keys = {
        ...
        'hyperliquid_price': 3,
        'average_price': 4,
        'link_pyth_drift_percent': 5,
        'pyth_oracle': 6,
        'big_drift': 7,
        'pyth_ema_trend': 8,
        'big_ema_drift': 9,
        'ema_drift_direction': 10,
    }
    ...
    async def monitor_queue(self):
        while True:
            ...
            await asyncio.sleep(1)  # Adjust the sleep time for faster pull. RPC will be LOADED
    ...
    def on_mount(self) -> None:
        self.add_note("HampTerminal is running")
        try:
...
#program pre-starter
# Input Args
parser = argparse.ArgumentParser(description='HampTerminal')
...
if __name__ == "__main__":
    print(f'Hampterminal Spinning Up...')
    global all_market_data
